...
const agregarItemCarrito = async (id_carrito, id_producto) => {
  const query = `
    ...
    SET cantidad = LEAST(carritos_detalle.cantidad + 1, (SELECT stock FROM productos WHERE id_producto = EXCLUDED.id_producto)),
        subtotal = LEAST(carritos_detalle.cantidad + 1, (SELECT stock FROM productos WHERE id_producto = EXCLUDED.id_producto)) * EXCLUDED.precio_fijo
    RETURNING *;
  `;
...
const disminuirItemCarrito = async (id_carrito, id_producto) => {
  ...
  const deleted = await pool.query(
    `DELETE FROM carritos_detalle WHERE id_carrito = $1 AND id_producto = $2 AND cantidad = 1 RETURNING *;`,
    values
  );
...
const eliminarProductoDelCarrito = async (id_carrito, id_producto) => {
  const { rows } = await pool.query(
    `DELETE FROM carritos_detalle WHERE id_carrito = $1 AND id_producto = $2 RETURNING *;`,
...
const obtenerPedidosUsuario = async (id_usuario) => {
  ...
  for (const p of rows) {
    const items = await pool.query(
      `SELECT pd.id_detalle, pd.id_producto, pr.titulo, pr.url_imagen, pd.cantidad, pd.precio_fijo, pd.subtotal
...
const admin_borrarCarrito = async (id_usuario) => {
  const { rows } = await pool.query(
    `DELETE FROM carritos WHERE id_usuario = $1 AND estado = 'abierto' RETURNING *;`,
    [id_usuario]
  );
  return rows[0];
};
